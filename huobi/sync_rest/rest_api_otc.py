...
from .consts import OTC_REST, GET, POST, DELETE, EXCHANGE_NAME
...
class huobiRestOTC(Client):

    def __init__(self, api_key, secret_key, host=OTC_REST, exch="pro"):
        super(huobiRestOTC, self).__init__(api_key, secret_key, host, exch)
        # url: /v1/account/accounts return :"id": 10000001,"type": "spot"
        self.account_id_map = {}
        self.account_id_map_reverse = {}

   
    def get_quote(self, baseCurrency, quoteCurrency, side, baseCurrencySize=None, quoteCurrencySize=None):
        params = {}
        params["baseCurrency"] = baseCurrency
        params["quoteCurrency"] = quoteCurrency
        params["side"] = side
        if baseCurrencySize:
            params["baseCurrencySize"] = baseCurrencySize
        if quoteCurrencySize:
            params["quoteCurrencySize"] = quoteCurrencySize
        return self._request(f"/open/otc/api/order/rfq/quote/request", POST, params)
