...
class RestClient(object):
    ...
    * 重载on_error方法来实现请求异常的标准回调处理
    ...
    def request(
        ...
        """同步请求函数"""
        request: Request = Request(method, self.url_base, path, params, data, headers)
    ...
    async def _process_request(self, request: Request) -> None:
        """发送请求到服务器，并对返回进行后续处理"""
        try:
            print(f"{datetime.now()}  begin get response")
            response: Response = await self._get_response(request)
            status_code: int = response.status_code

            # 2xx的代码表示处理成功
            print(f"{datetime.now()}  call back")
            await request.callback(response, request)
